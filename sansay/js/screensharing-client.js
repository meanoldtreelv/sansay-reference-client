...
//var screenSharingExtensionId = "hmhpjflelbhaaimpibhleppnhajflldp";

// Make a simple request:
// chrome.runtime.sendMessage(screenSharingExtensionId, {getScreen},
//   function(response) {
...
var getScreenMedia = function( sourceId , success_cb , fail_cb) {
  var constraints = {
    mandatory: {
...
WebRTC.prototype.open = function(local_media_ready_cb, local_media_end_cb, ice_ready_cb, remote_media_change_cb, enable_video) {
  ...
  window.addEventListener("message", function(msg){
   if( !msg.data ) {
     ...
     getScreenMedia( msg.data.sourceId , _media_success_handler , _media_fail_handler);
   } else if( msg.data.addonInstalled ) {
      console.log('addonInstalled');
...
var screensharing_logger = new Logger("ScreenSharing");
    ...
    if (data.type === "SESSION") {
      ...
      if (data.session !== undefined) {
        if (data.session.messageType === "SUCCESS") {
          ...
          screensharing_logger.debug("login successfully")
        }
          ...
          screensharing_logger.warning("receive handled SESSION message of " + data.session.messageType);
        }
      }
      else {
        screensharing_logger.error("Malformed SESSION message received");
      }
    }
    else if (data.type === "CALL") {
      //screensharing_logger.debug("got call with roap\n" + JSON.stringify(data.roap))
      if (data.roap !== undefined && data.roap.messageType === "OFFER") {
        screensharing_logger.debug("got OFFER from conf server with SDP");
        screensharing_logger.debug(data.roap.sdp);
        ...
        if (data.roap.info === "PARTICIPANT_STREAM") {
          ...
          if (stream_cnt > msid_cnt) {    // try again for missing ones
            screensharing_logger.debug("PARTICIPANT_STREAM: " + JSON.stringify(data.roap.data));
            screensharing_logger.debug("streams: " + JSON.stringify(stream_ids));
            screensharing_logger.debug("dict: " + JSON.stringify(dict));
            screensharing_logger.debug("** try again **");
        ...
        screensharing_logger.debug("receive ERROR of " + data.roap.errorType);
        ...
        screensharing_logger.debug("receive OK. This is for SUBSCRIBE we sent")
      }
      else {
        screensharing_logger.warning("we only expect OFFER. however we are getting " + data.roap.messageType);
        // what to do?
      }
    }
  };
...
SansayConfClient.prototype.login = function(user_id, pwd, cname, domain, success_cb, fail_cb) {
  ...
  this.sigChan.open(this.signaling_server, this.signaling_controller_path, user_id, function(status) {
    if (status === "connected") {
      screensharing_logger.debug("connect successfully to server");
      if (typeof success_cb === "function")
        success_cb();
    }
    else if (status === "disconnected") {
      screensharing_logger.error("disconnected from server");
      if (typeof fail_cb === "function")
        fail_cb()
    }
...
SansayConfClient.prototype.join = function(room, in_cb, out_cb, update_cb) {  // room_id should also identify the SFU and room #
  ...
  this.media.open(
    function() {
      if (conf_ready) {
        screensharing_logger.debug("conf is ready");
      }
      else {
        self.media.joinGroup(function(sdp) {
          screensharing_logger.debug("sending SUBSCRIBE with SDP");
          //screensharing.logger.debuf(sdp.sdp);
